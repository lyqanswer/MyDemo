package com.edu.mydemolist.bearsms;
...
public class BearSMSCaptchaActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_bearsmscaptcha);
		initData();
		setupViews();
	}
	...
	private void countDown() {
		new Thread(new Runnable() {
			public void run() {
				while (time-- > 0) {
					final String unReceive = BearSMSCaptchaActivity.this.getResources().getString(R.string.smssdk_receive_msg, time);
	...
	public void onClick(View view) {
		...
		case R.id.btn_next:
			 ...
			 * 解析错误;-5初始化异常 reason 返回信息 成功或错误原因. result 返回结果,JSON格式.错误或者无返回值时为空.
			 * 
			 */
		...
		case R.id.btn_submit:
			...
			mCaptcha.commitCaptcha(phoneNumber, code, new ResultCallBack() {
				...
				public void onResult(int code, String reason, String result) {
					...
					if (code == 0) {
						// 短信验证成功
						Toast.makeText(BearSMSCaptchaActivity.this, "验证成功！！！！！！！！", Toast.LENGTH_LONG).show();
					} else {
						// 验证失败
						Toast.makeText(BearSMSCaptchaActivity.this, "失败！！！！！！！！" + reason, Toast.LENGTH_LONG).show();
					}
